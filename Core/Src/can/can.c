...
void CAN_Filter_and_Start(uint16_t device_address) {
	...
	canFilterConfig.FilterIdHigh = (uint32_t)(CAN_COLOR_IDS_OFFSET + (device_address << 3)) << 5; 	// ID #1
	canFilterConfig.FilterIdLow = (uint32_t)(CAN_COLOR_IDS_OFFSET + (device_address << 3)) << 5;    // ID #2
	canFilterConfig.FilterMaskIdHigh = 0x07F8<<5;   												// Mask #1
	canFilterConfig.FilterMaskIdLow = 0x07F8<<5;    												// Mask #2
...
void CAN_Config() {
	if (GlobalConfig.config.CAN_Address > 0) {
		...
		CAN_Filter_and_Start(DEFAULT_DEVICE_CAN_ADDRESS);
	}
